/**
 * Bollinger Bands — standard 20-period SMA ± 2σ on closes — plus the odds of
 * touching one band before the other.
 */
...
const EMPTY: Bands = { upper: [], mid: [], lower: [] };
...
export function computeBands(
  ...
  { period = 20, mult = 2 }: { period?: number; mult?: number } = {},
): Bands {
  const n = bars.length;
  ...
  for (let k = period - 1; k < n; k++) {
    const window = closes.slice(k - period + 1, k + 1);
    const mu = mean(window);
    const sd = stdev(window, mu);
    const time = bars[k].time;
    upper.push({ time, value: mu + mult * sd });
    mid.push({ time, value: mu });
    lower.push({ time, value: mu - mult * sd });
  }
