...
import { useAgenda, useAgendaAlert } from "@/hooks/useAgenda";

const TABS = [
  { href: "/", label: "Dashboard" },
  { href: "/qa", label: "Q&A" },
  { href: "/bands", label: "Bandas" },
  { href: "/players", label: "B3" },
  { href: "/cme", label: "CME" },
...
export function Header({
  ...
  // Here so the news alert rings on every tab, not only the one with the panel.
  const agenda = useAgendaAlert(useAgenda());

  return (
    <>
      {agenda.alert && <AgendaAlert alert={agenda.alert} onDismiss={agenda.dismiss} />}
