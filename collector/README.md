...
| **M5 candles (Bandas tab)** | `copy_rates_from_pos` M5 (read-only), bar times converted to true UTC | ✅ real broker bars — the same candles your MT5 chart draws; the dashed band projection on top is an extrapolation computed in the frontend |
...
| `candles_poll_seconds` | How often (s) to push the **M5 candle history** that feeds the frontend's Bandas tab (Bollinger + projected continuation). Default `5`. `0` disables. |
| `candles_bars` | How many M5 bars to send per push (default `120`, newest last — the final bar is the one still forming). |
